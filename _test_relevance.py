...
⚠️ 本脚本曾长期处于「必然失败」状态：断言写死于早期实现，而后实现演进、测试未同步。
   本次仅将断言对齐当前实现，**未改动任何生产逻辑**。修正点：
   1) 相关性过滤的兜底阈值已抽为 daily_report.MIN_KEPT_ITEMS（=20）。
      旧断言假定「5 条样本会保留 2 条」，忽略了「保留数低于阈值则整体取消过滤」的兜底，
      故小样本下必然断言失败。
   2) 摘要策略为「保证每条都有摘要」：无原文时基于标题生成规则摘要
      （见 _fallback_rule_summary 的最终校验分支）。旧断言按「无原文保持空」编写，与现行为不符。
   因此测试 1 改为「大样本验证过滤生效 + 小样本验证兜底」两段，测试 2 改为「无原文也必须产出摘要」。
"""
import os
import sys
